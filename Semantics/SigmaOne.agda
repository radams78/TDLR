...
p₁ _ = record { ap-vertex = proj₁ ; ap-path = proj₁ ; ap-fill = λ fill → fill ; ap-fill₂ = λ fill₂ → fill₂}

q₁ : ∀ {G} {S} → Section₁ {Sigma₁ G S} (pullback₁ (p₁ S) S)
q₁ = record { vertex = proj₂ ; edge = proj₂ }

pair₁-cong : ∀ {G H S} {F F' : Groupoid-Functor G H} {s : Section₁ (pullback₁ F S)} {t : Section₁ (pullback₁ F' S)} →
  (α : Groupoid-NatIso F F') → Section₀ (EQ₁ s (pullback₁-congl α S) t) → Groupoid-NatIso (pair₁ S F s) (pair₁ S F' t)
