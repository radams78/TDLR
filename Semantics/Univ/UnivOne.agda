...
eq₂-cong : ∀ {A A' B B'}
  ...
  (A* : T₂ (Eq₂ A A')) (B* : T₂ (Eq₂ B B')) (e : T₂ (Eq₂ A B)) (e' : T₂ (Eq₂ A' B')) →
  T₁ (eq₂ a A* a') → T₁ (eq₂ e (Eq₂-cong A* B*) e') → T₁ (eq₂ b B* b') →
  T₁ (Eq₁ (eq₂ a e b) (eq₂ a' e' b'))
eq₂-cong {a = a} {a'} {b} {b'} _ _ _ _ a* e* b* = app₁ (app₁ (App₁ (App₁ (App₁ (App₁ e* a) a') b) b') a*) b*
...
Eq₂-cong₂ {A* = A*} {B*} {C*} {D*} {AC = AC} {A'C'} {BD} {B'D'} A*C* B*D* = Λ₁ (λ AB → Λ₁ (λ CD → Λ₁ (λ A'B' → Λ₁ (λ C'D' →
  lam₁ (λ AB∼CD → lam₁ (λ A'B'∼C'D' →
  Pi₁* AC (λ a c ac →
  Pi₁* A'C' (λ a' c' a'c' →
  Pi₁* BD (λ b d bd →
  Pi₁* B'D' (λ b' d' b'd' →
  ⇒₁* (eq₂-cong AC A'C' A* C* ac A*C* a'c')
  (⇒₁* (eq₂-cong BD B'D' B* D* bd B*D* b'd')
  (Eq₁-cong (eq₂-cong AC BD AB CD ac AB∼CD bd) (eq₂-cong A'C' B'D' A'B' C'D' a'c' A'B'∼C'D' b'd')))))))))))))
